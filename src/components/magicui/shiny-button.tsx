...
const ShinyButton = ({ children, className, onClick }: ShinyButtonProps) => {
  return (
    <motion.button
      ...
      onClick={onClick}
      className={cn(
        "relative rounded-lg px-4  py-3 font-medium backdrop-blur-xl transition-[box-shadow] duration-300 ease-in-out  dark:bg-[radial-gradient(circle_at_50%_0%,hsl(var(--primary)/10%)_0%,transparent_60%)] dark:hover:shadow-[0_0_20px_hsl(var(--primary)/10%)]",
        className
      )}>
      <span
        className="relative block h-full w-full text-sm capitalize tracking-wide text-[rgb(0,0,0,65%)] dark:font-light dark:text-[rgb(255,255,255,90%)]"
        style={{
          maskImage:
